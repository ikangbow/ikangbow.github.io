...
## 策略接收行情
    ...
    作者：<em>AlgoPlus</em>
链接：https://zhuanlan.zhihu.com/p/86082225
来源：知乎
...
3、Join方法中的策略每次执行时，从队列中取出所有数据，以最后一笔行情的盘口价格作为委托价。4、OnRtnOrder收到订单状态通知时更新本地订单状态、持仓手数，在策略中根据状态的变化进行后续操作。
...
4、所有进程的join方法须在start方法之后最后调用。5、这段代码放置在策略代码最后，执行即可看到执行结果。6、参考这个例子可以很方便的扩展一对多、多对多的进程间数据共享模式。
